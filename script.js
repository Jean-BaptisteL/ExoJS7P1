function strangeCalcul(){
  ...
  if((isNaN(footSize) == true) || (isNaN(year) == true)){ //Si les valeurs entrées ne sont pas des nombres, un message d'erreur apparait.
    alert('Veuillez entrer un nombre pour la pointure et l\'année de naissance');
  }else if((parseFloat(footSize) < 16) || (parseFloat(footSize) > 50) || (parseFloat(year) < 1900) || (parseFloat(year) > 2019)){ //Si les valeurs ne sont pas comprises entre ces intervalles, un message d'erreur apparait.
